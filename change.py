...
def generate_voiceover(text, language='en', gender='male'):
    engine = pyttsx3.init()
    if gender == 'male':
        engine.setProperty('voice', 'english_male')  # Change this to a male voice in your language
    else:
        engine.setProperty('voice', 'english_female')  # Change this to a female voice in your language
    
    engine.save_to_file(text, 'temp_voiceoverpp.mp3')
    engine.runAndWait()
...
def main():
    ...
    audio_path = r'C:\Users\DELL\OneDrive\Desktop\translate\temp_audio2.wav'
    output_path = r'C:\Users\DELL\OneDrive\Desktop\translate\output_video2.mp4'

    extract_audio(video_path, audio_path)
    text = speech_to_text(audio_path)
    translated_text = translate_text(text)
    generate_voiceover(translated_text, language='gu', gender='male')

    create_translated_video(video_path, translated_text, output_path)

    # Optional: Clean up temporary files
    os.remove(audio_path)
    os.remove('temp_voiceover1.mp3')
